...
import { canvas, ctx, level, gameOver, score, highScore } from './src/gameCore.js';
import { updateHealthBar, updatePlayerImage, updateScore } from './src/hud.js';
import { player } from './src/entities.js';
import { bullets } from './src/entities.js';
import { playExplosion, playShieldDown } from './src/sound.js';
...
(function () {
  ...
  window.spawnMeteor = function (playerX = canvas.width / 2, playerY = canvas.height / 2) {
    ...
    meteors.push({ x, y, width: size, height: size, speed: vy, vx, vy, hp, img, lastShotTime: Date.now(), isRogue: isRogueMeteor });
  };

  window.drawMeteors = function () {
    for (let mi = meteors.length - 1; mi >= 0; mi--) {
      ...
      for (let bi = bullets.length - 1; bi >= 0; bi--) {
        ...
        if (b.x < m.x + m.width && b.x + b.width > m.x && b.y < m.y + m.height && b.y + b.height > m.y) {
          ...
          if (m.hp <= 0) {
            let meteorPoints = m.isRogue ? 50 : (m.hp > 3 ? 30 : m.hp > 2 ? 15 : 5);
  ...
  window.checkMeteorCollisions = function (playerHP, setGameOver, shakeTimerRef, timerDisplay) {
    meteors.forEach((m, mi) => {
      if (
        ...
        meteors.splice(mi, 1);
        playerHP--;
        shakeTimerRef.value = 120;
        updateHealthBar(playerHP);
        updatePlayerImage(playerHP);
        playShieldDown();
